    ...
    <div x-show="open" x-cloak @keydown.escape.window="open = false"
        ...
        <div class="relative w-full max-w-[400px] mx-auto mt-16 bg-white rounded-lg shadow-lg overflow-hidden max-h-[70vh]">
            ...
            <div class="max-h-[58vh] overflow-y-auto">
                @if (!auth()->user()->hasVerifiedEmail() && ($notifications ?? collect())->isEmpty())
                            ...
                            <li class="px-4 py-3">
                                <a href="{{ $notification->link ?? '#' }}" class="block">
                                    <div class="flex items-start justify-between">
                                        <div class="flex-1">
                                            <div class="text-sm {{ $notification->is_read ? 'text-gray-700' : 'font-semibold text-gray-800' }} truncate">{{ $notification->content }}</div>
                                            <div class="text-xs text-gray-500 mt-1">{{ $notification->sent_at ? \Carbon\Carbon::parse($notification->sent_at)->diffForHumans() : '' }}</div>
                                        </div>
            ...
            <div class="px-4 py-3 border-t text-right">
                <a href="{{ route('student.notification-view') }}" class="text-sm text-[#660809] hover:underline">View all</a>
            </div>
        </div>
    </div>
</div>
